...
- **Phase 5 — package move to `flags.*` → rename to `wunmi` root package.** Mechanical; sets up jar extraction. No behavior change. Also: purify `FlagStore` to plain records (the pre-extraction debt).
